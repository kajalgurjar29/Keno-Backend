...
const killZombieChromium = async () => {
  try {
    // Only Windows Cleanup. No pkill on Linux.
  ...
  const runScraperOnce = async () => {
    ...
    try {
      ...
      page.on('request', interceptedRequest => {
        ...
        if (url.includes('api-info') && (url.includes('jurisdiction=ACT') || url.includes('jurisdiction=NSW') || url.includes('jurisdiction=VIC'))) {
          const newUrl = url.replace(/api-info-(act|nsw|vic)/, 'api-info-sa').replace(/jurisdiction=(ACT|NSW|VIC)/, 'jurisdiction=SA');
      ...
      // 🔁 Switch to SA region (if not already there)
      try {
        ...
        console.warn("⚠️ SA Region switch failed:", e.message);
      }
      ...
      console.log(`📊 SA: Scraped ${games.length} games from the page.`);
      ...
      } catch (ae) { console.warn("⚠️ SA Live API fetch failed:", ae.message); }
      ...
      for (const game of games) {
        ...
        try {
          ...
          if (!existing) {
            ...
            console.log(`🔧 SA repaired malformed draw ${game.draw}`);
          }
      ...
      console.error("❌ SA Scraper runScraperOnce failed:", err.message);
      await safeClose(browser);
      throw err;
    }
  };
