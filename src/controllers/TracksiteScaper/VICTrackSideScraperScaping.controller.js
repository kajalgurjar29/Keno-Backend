...
const killZombieChromium = async () => {
  try {
    ...
    if (process.platform === "win32") {
      ...
      console.log("🧹 VIC: Cleaned up zombie processes (Windows)");
    }
  } catch (err) {
    if (process.platform === "win32") {
...
export const getFilteredTrackSideResults = async (query = {}) => {
  try {
    ...
    console.error(
      "❌ VIC: Error fetching filtered TrackSide results:",
      err.message
    );
...
export const getPaginatedTrackSideResultsOnly = async (query = {}) => {
  try {
    const { location = "VIC", limit = 10, page = 1 } = query;
    ...
    console.error("❌ VIC: Error fetching paginated results:", err.message);
    return { data: [], totalCount: 0, totalPages: 0, currentPage: 1 };
  }
};
...
export const getFilteredTrackSideResultsOnly = async (query = {}) => {
  try {
    ...
    console.error("❌ VIC: Error fetching filtered results:", err.message);
    return { data: [], totalCount: 0, totalPages: 0, currentPage: 1 };
  }
};
